...
본 파이프라인은 아키텍처 v2.0([`06`](06_아키텍처v2_제로존방법론통합백서.md))의 **Phase 4(J-Executive Check)** 와 정합한다.
집행관(Agency-J)의 출간 자격 종합 점검표 = ① Sequence 일치 ② 금지어 Grep([`lint_text.py`](../apps/ebook_publisher/scripts/lint_text.py)) ③ Cross-Ref(Lemma) 일치 ④ 600 체크리스트 `complete`.
네 가지가 모두 PASS여야 변환·출판 산출물이 나온다.
...
> **선행 필수 게이트**: 변환(Builder) 진입 전에 윤문기의 600항목 체크리스트(`checklist_run.json`)가
> `complete=true`(`pending==0 && fail==0`)여야 한다. 미완료면 `ComplianceGate.checklist_incomplete`로 **차단**된다.
> 상세는 `02_윤문기엔진_상세설계서.md` §1.1.

```mermaid
flowchart TD
  CHK["[필수] 600항목 체크리스트 complete?"] -->|아니오| BLK["차단: checklist_incomplete"]
  CHK -->|예| R["refined/final.md + meta_candidates"]
  R --> B["Builder 변환"]
...
| 게이트 | 조건 | 동작 |
|---|---|---|
| `checklist_incomplete` | **600항목 윤문 체크리스트 미완료**(`pending>0` 또는 `fail>0`) | **차단** + 미완료 항목 목록 |
| `hard_fail_lint` | 금지어/괄호·마크다운 깨짐/빈 청크(`lint_text`) | **차단** + Agency-G 반송(재작업) |
| `honest_box_open` | 미결재 `◑` 마커 존재(v2.0) | **차단** + ◑ 결재 요청 |
| `cross_ref_desync` | Lemma 연쇄 갱신 미반영(v2.0) | **차단** + 동기화 큐 처리 요청 |
