...
## 1.2 v2.0로의 진화 (제로존 방법론)

본 문서(v1)는 단일 파이프라인 규칙엔진 윤문기를 정의한다. 대형·고정합 원고를 위한
**다중 에이전트·다중 사이클·정직 봉인·메타 좌표 동기화** 확장은
[`06_아키텍처v2_제로존방법론통합백서.md`](06_아키텍처v2_제로존방법론통합백서.md)에서 정의한다.

- v1 단계(교정/문체/일관성/가독성)는 v2.0에서 **역할별 에이전트**(Agency-G/F/E/A·B·C)로 분해된다.
- 본 문서의 제안 `severity`(auto/review/manual)는 v2.0의 정직 봉인 마커와 매핑된다:
  `auto`→`★`, `review`/`manual`→`◑`(인간 결재 필요).
- Hard-Fail 게이트는 실제 린터 [`../apps/ebook_publisher/scripts/lint_text.py`](../apps/ebook_publisher/scripts/lint_text.py)로 구현되어 있으며, 모든 윤문 산출 청크는 이 린터를 통과해야 한다.
- 소형 원고는 v1(규칙엔진)으로 충분하고, `engine.mode = "v2_multiagent"`일 때만 에이전트/그래프 계층을 켠다.
