...
2. 교정 규칙엔진(띄어쓰기·맞춤법·부호) + report.json/diff.html.
3. 수락/거절 반영 → refined.md 생성, `/api/refine` 연결.
4. 문체·용어·가독성 모듈, 사용자 사전/프로파일.
5. 메타 추출 → 뒷단 출판 자동 연결.
6. (옵션) 로컬/외부 LLM 하이브리드.
